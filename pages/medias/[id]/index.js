...
import {createComment, listComments, updateComment} from '../../../media/apiComments';
...
const Media = props => {
	...
	const listingComments = () => {
		return comments.map(comment => {
	          ...
	          {editMode ? (
	          	<div className='mb-5'>
	          		<form className='form-group' onSubmit={handleUpdateComment}>
	          			<textarea className='form-control' name="text" onChange={handleChangeText} cols="30" rows="10"></textarea>
	          			<Button variant='contained' onClick={handleUpdateComment} color='secondary'>
										Update
		            	</Button>
	          		</form>
	          	</div>
	        ...
	        <Grid item>
	            {isAuthenticated() && isAuthenticated().user._id === comment.author._id && (	
	            	<div>
	            	{!editMode && (
            				<Button variant='contained' onClick={handleEditComment} color='secondary'>
											<EditIcon/>
			            	</Button>		
            			)}
	...
	const nextUrl = props.related.length > 0 ? `medias/${props.related[0]._id}` : '';
	return(
		<div className='container-fluid'>
			<div className='row'>
				<div className='col-md-8'>
					<MediaPlayer title={props.video.title} nextUrl={nextUrl} srcUrl={`http://localhost:8000/media/video/${props.videoId}`} />
					<h3>Comments</h3>
